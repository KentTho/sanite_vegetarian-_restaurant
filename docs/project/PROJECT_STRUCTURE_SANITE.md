...
- **Lotus backdrop panel (Story):** ảnh thật `assets/hoasen.jpg` qua `<div className="story-lotus-photo story-lotus-backdrop"><img alt="" aria-hidden/></div>`. Là **khối ảnh lớn ở góc DƯỚI–TRÁI Story** (Prompt 08b: `position:absolute; left:0; bottom:0; top:auto` → sát cạnh trái + sát đáy Story); `width clamp(360–620px)`, `height clamp(620–900px)`; `object-fit:cover` (object-position center 42%); `border-radius:0 46px 0 0` (vuông góc trái & đáy); `mask-image: linear-gradient(to right, …)` mờ dần sang phải; `box-shadow` mềm; opacity ~0.5 (mobile watermark 0.36–0.42, cũng sát đáy–trái). Nằm sau chữ (`z-index:0`), có **scrim** `.story-left::before` (gradient kem) bảo vệ readability → KHÔNG che chữ. `.story` `overflow:visible`; `body{overflow-x:hidden}` chống tràn. `LotusBranch` SVG vẫn còn trong hooks nhưng không render.
