...
- **Lotus backdrop panel (Story):** ảnh thật `assets/hoasen.jpg` qua `<div className="story-lotus-photo story-lotus-backdrop"><img alt="" aria-hidden/></div>`. Là **khối ảnh bên PHẢI Story, cao bằng Story** (Prompt 08c: `position:absolute; right:0; top:0; bottom:0; left:auto` → KHÔNG lấn lên Hero); `width clamp(320–560px)`; `object-fit:cover` (object-position center 42%); **KHÔNG viền/khung/box-shadow** (Prompt 08d — bỏ border, hòa quyện vào nền): `mask-image: radial-gradient(135% 108% at 100% 48%, …)` làm ảnh đậm ở mép phải và mờ tan các rìa trái/trên/dưới vào nền kem; opacity ~0.5 (tablet 0.4, mobile 0.3). Nằm sau cards (`z-index:0`), cards giữ bg riêng nên vẫn đọc rõ; giữ nguyên tông màu. `.story` `overflow:visible`; `body{overflow-x:hidden}` chống tràn. Scrim `.story-left::before` giữ nguyên (vô hại). `LotusBranch` SVG vẫn còn trong hooks nhưng không render.
